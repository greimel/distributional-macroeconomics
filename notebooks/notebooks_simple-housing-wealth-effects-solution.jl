...
md"""
`simple-housing-wealth-effects-solution.jl` | **Version 1.1** | *last updated: May 31 2023*
"""
...
(1) Which parameter are set according to external information and which are calibrated "internally"? (With internal calibration I mean to choose parameter values according to some method of moments algorithm.)

In my opinion, it makes a lot of sense to use external information to set parameter values whenever possible. The remaining parameters which need to be calibrated internally are often related to preferences.

(2) Which moments should be selected for "internal" calibration?

A good starting point is to pick moments such that each moment is particularly informative about one parameter. (Of course, in the method of moments algorithm all moments determine jointly the parameter values.)
...
md"""
👉 **External validity** (3 points) | A politician asks you to forecast the consumption response to house price shock in 2030. Would you use the given model? Discuss some aspects of the real world that are missing from the model. The figure below provides two hints that you may pick up or ignore.
"""
...
- (1) it captures the covariation of housing wealth with age well
- (2) it captures the covariation of MPCs with age well

(1) The two figures below show that according to survey data older households tend to own more housing wealth than younger households, both because they are more likely to be homeowners and because they tend to live in more expensive houses (conditional on being homeowners). In contrast, housing wealth is flat over the life-cycle in the model. Therefore, the model might underestimate the effect of the demographic structure on the consumption response to house price shocks. Adding a rent vs. buy choice and a loan-to-value constraint to the model should help to match the data better.

(2) The model predicts that MPCs increase with age, in particular among old retirees. It would be a good idea to find some empirical evidence regarding the relationship of MPCs with age to check if this is actually true. Moreover, it seems as if the average MPC in the model is much lower than in the empirical literature on MPCs. For this reason, it would make sense to add some model features which increase the overall level of MPCs, such as income risk and housing adjustment costs.
"""
...
manifest_format = "2.0"
project_hash = "0b6e145019d0ef8a4b204b0df9ff423975c98ef0"

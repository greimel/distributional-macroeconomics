...
function Y(H, h, K, l; η = η, α = α, ρ = ρ, A = A)
	intan = H^α * h^(1 - α)
	tan = K^α * l^(1 - α)

	agg = η * intan^ρ + (1 - η) * tan^ρ
...
let
	h, l = 1, 1
	H = K = range(0.01, 1, length = 101)

	title = latexstring("Contourf plot of \$Y(H, h = 1, K, l = 1)\$")
	
	contourf(
		H, K, (x, y) -> Y(x, h, y, l),
		title = title,
		xlabel = L"H", ylabel = L"K"
	)
end
...
begin
	function computeprices(H, h, K, l; params...)
		f = x -> Y(x[1], x[2], x[3], x[4]; params...)
		∇Y = ForwardDiff.gradient(f, [H, h, K, l])
		R, q, r, w = ∇Y
...
let
	...
	ηprice = η -> computeprices(c, c, c, c; η = η)
	prices = hcat(collect.(ηprice.(ηs))...)'

	ktitle = latexstring("Price of capital, for different \$\\eta\$")
	ltitle = latexstring("Price of labour, for different \$\\eta\$")
	

	kfig = plot(
		ηs, prices[:, 1], label = L"R", xlabel = L"\eta", 
		title = ktitle
	)
	plot!(ηs, prices[:, 3], label = L"r")

	lfig = plot(
		ηs, prices[:, 2], label = L"q", xlabel = L"\eta", 
		title = ltitle
	)
	plot!(ηs, prices[:, 4], label = L"w")

	s =  300 .* [2 * √2, 1]
	plot(kfig, lfig, size = s, margin = 6Plots.mm)
	
end

# ╔═╡ 7b6726b6-8288-406a-b050-e7bf04466220
md"
...
begin
	v(L) = L^2
	v′ = L -> ForwardDiff.derivative(v, L)
	
	function pₜ₊₁(pₜ, Lₜ, rₜ₊₁)
		rₜ₊₁ * pₜ - v′(Lₜ)
	end

	function fₜ₊₁(fₜ, dₜ, rₜ₊₁)
		rₜ₊₁ * fₜ - dₜ
	end

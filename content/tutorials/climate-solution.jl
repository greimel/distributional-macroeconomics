...
# ╔═╡ 7ab93492-c9d7-488f-92d2-cea41644b333
using Roots: find_zero, Brent

# ╔═╡ e235e2f2-0383-4bf6-8530-9a17d339fa20
using Optim: optimize

# ╔═╡ 27c6314b-004a-4168-b8b9-cf387ce5d756
using Statistics: mean

# ╔═╡ f0a15739-cd22-4eed-bb3a-0af6857b2000
...
&\begin{aligned}
	\text{subject to } 
		&u(c, d) = \log(c^\eta (d-\underline{d})^{1-\eta}) \\
		&k_t \ge 0 \\
...
As a first step, let's consider a simple static model without income risk and without the ad hoc borrowing constraint $k_t\ge 0$. ($e$ denoutes the non-stochastic income.)
...
The combined first order condition:

$$\frac{1-\eta}{\eta} \frac{c}{d-\underline{d}} = p_d + \tau_d $$

Then if we plug this condition into the budget constraint we get the following demand system:	

$$c = \eta (e - (p_d + \tau_d) \underline{d})$$

$$d = (1-\eta) \frac{e}{p_d + \tau_d} + \eta \underline{d}$$

The Engel curve tells us how the demand varies with income. From the demand equations that we found we can clearly see that they are linear in income $e$.

The linearity does not hold in the full model because of the precautionary savings motive. Because of the borrowing constraint the agents with little wealth have stronger incentives to save and therefore to spend less on consumption today. **(Formally: $e$ is not linear in income)**
...
👉 Use the results of the static model to modify the reward function to solve the model.
"""

# ╔═╡ 813e9358-cb27-4f5b-b174-646ffd2e7f55
md"""
**Answer**
Since there is a closed-form solution to clean/dirty good demand given the consumption expenditure, we can stil use the standard code. The difference is that the consumption policy function now denotes the consumption expenditure and does not correspond to the utility that the agents derive. To divide the expenditure across the two goods we can use the expressions from the previous exercise:
...
In order to close the model we need to find the equilibrium price for the dirty good. Assume that there is an endowent and the price should make the market clear:
...
md"""
**Answer**
 - An increase in tax increases the relative price of the dirty good compared to the clean one. Therefore the social cost of dirty good consumption goes down.
 - Wealthier households are consuming more dirty good and therefore carry higher burden of the taxation. Since the lump sum taxes are distributed equally, this tax is a tool to redistribute wealth to the poorer.

The other way:
 - The higher the tax the more distorted is the household decision and lower their utility.
"""
...
function reward(state, policy, prices, u)
	...
	c = η*(c_exp - (prices.pd + prices.τd)*d_under)
	if c < 0
...
function results_to_df(results, states, policies, prices)
	...
	df.consumption_c = η*(df.consumption .- (prices.pd + prices.τd)*d_under)
...
function net_dirty_demand(hh, ss, pd)
	prices  = (q = 1/(1+r), pd = pd, τd = 0)
	df = solve_PE(hh, ss, prices)
...
function government_deficit(hh, T, τd)
	df_τ = solve_GE(hh, ss, prices, T, τd)
...
function welfare(τd)
	...
	df_τ = solve_GE(hh, ss, prices, T_eq, τd)
	welfare = mean(df_τ.value,   weights(df_τ.π)) - θ*(mean(df_τ.consumption_d, weights(df_τ.π)))^2
end

# ╔═╡ 4e522395-865c-4f42-9ec3-ee2d4449edcd
begin
	tau_arr = range(0., 0.5, length = 25)
	W = welfare.(tau_arr)
	lines(tau_arr, W, legend=false)
end

# ╔═╡ 36f45dbd-f57c-49a5-9164-14acc05653ec
optimize(x -> -welfare(x), 0.0, 0.3)
...
# ╔═╡ a7c05659-feb3-47e3-b42d-e1a8bf6e3cea
...
# ╔═╡ 5a9a554d-50ca-406b-b1d0-a34d9d7697d4
...
manifest_format = "2.0"
project_hash = "851971e2cda31eacbe236fee6b3a2e5d01451f1a"

...
# ╔═╡ dd3c01fa-767e-4552-8b22-fe93b457c06b
using CairoMakie, AlgebraOfGraphics, DimensionalData

# ╔═╡ 27c91a50-b810-43e1-8d01-fe465f5bb229
using Colors

# ╔═╡ fbe90bd9-3815-4c0c-822f-b99cb55edcda
using Interpolations, Statistics

# ╔═╡ e0b52fe2-b876-11ef-36ff-8d49658809bf
md"""
# Endogenous grid method
"""

# ╔═╡ e9280748-e4b3-4eb7-ad9a-ce721db25fc5
function Model(; alpha = 0.65, # productivity parameter
    ...
    return (; alpha, beta, γ, mu, s, grid_min, grid_max, grid_size, u,
            u_prime, u_prime_inv,
            f, f_prime, grid)
end

# ╔═╡ b1adcf7a-4d97-405b-9370-ba0ffa94340e
function coleman_egm(σ, k_grid, (; beta, u_prime, u_prime_inv, f, f_prime), shocks)
	#σ: y -> c 
	...
	# y ... savings (endogenous grid)
	# c ... consumption
	# k ... exogenous grid y - c
    y = k_grid + c

    # Update policy function and return
   	σ_new0 = LinearInterpolation(y, c, extrapolation_bc = Line())
    σ_new(x) = σ_new0(x)
    
	return σ_new
end

# ╔═╡ d549362f-36d9-4bb7-8134-a01078cc09de
function check_convergence(m, shocks, g_init, n_iter)
    k_grid = m.grid #
    σ = g_init
    f = Figure()
	plt = Axis(f[1,1], xlabel = L"savings $y$", ylabel = L"optimal consumption $c(y)$")

    for i in 1:n_iter
        new_σ = coleman_egm(σ, k_grid, m,
                            shocks)
        σ = new_σ
		c = σ.(k_grid)
        ...
        scatterlines!(plt, y, c,
   				color = (:darkgreen, i / n_iter)
   		)
    end
...
# ╔═╡ 6f345daf-9e4f-43a4-8001-4ded505cdbb9
mlog = Model(γ = 3, grid_size = 10, grid_max = 2.0)

# ╔═╡ 918ebcd0-64f2-4f60-808c-5da2245eea49
shocks = let
	n_shocks = 250     # Number of shock draws in Monte Carlo integral
	shocks = exp.(mlog.mu .+ mlog.s * randn(n_shocks))
end

# ╔═╡ e0ecc879-5031-4bde-9516-a0721809f8ea
c_star(y) = (1 - mlog.alpha * mlog.beta) * y

# ╔═╡ 4758b195-fb1b-42e9-b13b-18bfc67a81ef
let
	σ = coleman_egm(c_star, mlog.grid, mlog, shocks)

	σ.(mlog.grid)
end


# ╔═╡ 3e10b7dd-7181-4d03-b6e4-93e4803467f2
check_convergence(mlog, shocks, identity, 5)
...
manifest_format = "2.0"
project_hash = "d96906d1e2a77049e40efcb4b41ca3e65fea4156"
